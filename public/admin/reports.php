...
$anonymousFilter = $_GET['anonymous'] ?? 'all';
$search = $_GET['search'] ?? '';
...
try {
    ...
    $query = "SELECT r.*, 
              ...
              COALESCE(assigned_ad.full_name, assigned_pd.full_name, assigned_psd.full_name, assigned_gud.full_name) as assigned_name
              FROM reports r
              ...
              LEFT JOIN admin_details assigned_ad ON assigned_user.id = assigned_ad.user_id AND assigned_user.role = 'admin'
              LEFT JOIN police_details assigned_pd ON assigned_user.id = assigned_pd.user_id AND assigned_user.role = 'police'
              LEFT JOIN psychologist_details assigned_psd ON assigned_user.id = assigned_psd.user_id AND assigned_user.role = 'psychologist'
              LEFT JOIN general_user_details assigned_gud ON assigned_user.id = assigned_gud.user_id AND assigned_user.role = 'general_user'
    ...
    if ($anonymousFilter == 'yes') {
        $query .= " AND r.is_anonymous = 1";
    } elseif ($anonymousFilter == 'no') {
        $query .= " AND r.is_anonymous = 0";
    }
            ...
            <div class="page-header">
                ...
                <div>
                    ...
                    <?php if ($pendingCount > 0): ?>
                        <span style="margin-left: 15px; padding: 5px 12px; background: rgba(255, 167, 38, 0.2); color: #FFA726; border-radius: 8px; font-size: 12px; font-weight: 600;">
                    ...
                    <div class="filters-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <select name="anonymous">
                                <option value="all" <?php echo $anonymousFilter == 'all' ? 'selected' : ''; ?>>Semua</option>
                                <option value="yes" <?php echo $anonymousFilter == 'yes' ? 'selected' : ''; ?>>Anonim</option>
                                <option value="no" <?php echo $anonymousFilter == 'no' ? 'selected' : ''; ?>>Beridentitas</option>
                            </select>
                        </div>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="<?= BASE_URL ?>/admin/report-detail.php?id=<?= $report['id'] ?>" class="btn-small btn-view">Lihat</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
