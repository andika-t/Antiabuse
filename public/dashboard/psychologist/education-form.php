    ...
    try {
        ...
        if (empty($title) || empty($video_url) || empty($category)) {
            ...
            if ($isEdit) {
                ...
                if ($stmt->rowCount() > 0) {
                    header('Location: ' . BASE_URL . '/dashboard/psychologist/education.php?success=updated');
                    exit();
                } else {
                ...
                header('Location: ' . BASE_URL . '/dashboard/psychologist/education.php?success=created');
                exit();
            }
        }
...
if ($isEdit) {
    try {
        ...
        if (!$content) {
            header('Location: ' . BASE_URL . '/dashboard/psychologist/education.php?error=not_found');
            exit();
        }
    } catch(PDOException $e) {
        $error = 'Error: ' . $e->getMessage();
