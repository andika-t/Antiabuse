...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_report'])) {
    try {
        ...
        // Tetap simpan user_id meskipun anonim untuk tracking, tapi identitas tidak akan ditampilkan di UI jika is_anonymous = 1
        ...
        if (empty($report_type)) {
            $error = 'Pilih tipe laporan!';
        } elseif ($report_type == 'other' && empty($report_type_other)) {
            $error = 'Spesifikasi tipe laporan lainnya harus diisi!';
        } elseif (empty($title) || strlen($title) < 10 || strlen($title) > 255) {
            $error = 'Judul laporan harus 10-255 karakter!';
        } elseif (empty($description) || strlen($description) < 50) {
            $error = 'Deskripsi kejadian minimal 50 karakter!';
        } else {
            // Check if reports table exists
            try {
                $checkTable = $conn->query("SHOW TABLES LIKE 'reports'");
                if ($checkTable->rowCount() == 0) {
                    $error = 'Tabel reports belum dibuat. Silakan jalankan script database schema terlebih dahulu.';
                } else {
                    // Insert report
                    $stmt = $conn->prepare("
                        INSERT INTO reports (
                    ...
                    $stmt->execute([
                        $user_id, $report_type, $report_type_other, $title, $description,
                        $location, $incident_date, $incident_time, $is_anonymous ? 1 : 0,
                    ...
                    if (!empty($_FILES['attachments']['name'][0])) {
                        $uploadDir = '../../../uploads/reports/' . $report_id . '/';
                        if (!is_dir($uploadDir)) {
                                ...
                                if (in_array($fileType, $allowedTypes) && $fileSize <= $maxSize) {
                                    ...
                                    if (move_uploaded_file($tmpName, $filePath)) {
                                        $stmt = $conn->prepare("
                                            ...
                                            VALUES (?, ?, ?, ?, ?, 'user_upload')
                                        ");
                                        $relativePath = 'uploads/reports/' . $report_id . '/' . $newFilename;
                                        $stmt->execute([$report_id, $filename, $relativePath, $fileSize, $fileType]);
                                    }
                                }
                            }
                        }
                    ...
                    header('Location: ' . BASE_URL . '/dashboard/user/report-success.php?id=' . $report_id);
                    exit();
                }
            } catch(PDOException $e) {
                // Check if error is about table not found
                if (strpos($e->getMessage(), "doesn't exist") !== false || strpos($e->getMessage(), "Table") !== false || strpos($e->getMessage(), "Unknown table") !== false) {
                    $error = 'Tabel reports belum dibuat. Silakan jalankan script database schema terlebih dahulu. Error: ' . $e->getMessage();
                } else {
    ...
    <script>
        ...
        function cancelReport() {
            if (confirm('Apakah Anda yakin ingin membatalkan pembuatan laporan? Semua data yang sudah diisi akan hilang.')) {
                clearDraft();
                window.location.href = '<?php echo BASE_URL; ?>/dashboard/user/index.php';
            }
        }
