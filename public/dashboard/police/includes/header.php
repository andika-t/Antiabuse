<?php
// Header component for police dashboard
require_once '../../../config/database.php';

$currentPage = basename($_SERVER['PHP_SELF']);

// Get unread panic notifications count
$unreadPanicCount = 0;
try {
    ...
    $result = $stmt->fetch();
    $unreadPanicCount = $result['count'] ?? 0;
...
(function() {
    function initNavIndicator() {
        ...
        if (activeLink) {
            ...
            indicator.style.transition = 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)';
        } else {
...
function loadNotifications() {
    fetch('<?php echo BASE_URL; ?>/dashboard/police/notifications.php')
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('notificationList').innerHTML = 
                    '<p style="text-align: center; padding: 20px; color: var(--color-text-light);">Error memuat notifikasi</p>';
            }
        })
        .catch(error => {
...
function displayNotifications(notifications) {
    ...
    if (notifications.length === 0) {
        container.innerHTML = '<p style="text-align: center; padding: 20px; color: var(--color-text-light);">Tidak ada notifikasi</p>';
        return;
    }
    
    container.innerHTML = notifications.map(notif => `
        <div class="notification-item ${notif.read_at ? '' : 'unread'}" 
             onclick="viewPanicAlert(${notif.id})">
            <div class="notification-content">
                <strong style="display: inline-flex; align-items: center; gap: 6px;"><?php echo icon('panic', '', 18, 18); ?> Panic Alert</strong>
                <p style="font-size: 12px; margin: 5px 0; color: var(--color-text);">
                    Dari: ${notif.user_name || 'User'}
                </p>
                <p style="font-size: 11px; color: var(--color-text-light);">
                    ${new Date(notif.created_at).toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}
                </p>
                ${notif.location ? `<p style="font-size: 11px; color: var(--color-text-light); margin-top: 5px; display: flex; align-items: center; gap: 4px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 10C21 17 12 23 12 23C12 23 3 17 3 10C3 5.02944 7.02944 1 12 1C16.9706 1 21 5.02944 21 10Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="10" r="3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg> ${notif.location}</p>` : ''}
            </div>
        </div>
    `).join('');
}

function viewPanicAlert(alertId) {
    window.location.href = '<?php echo BASE_URL; ?>/dashboard/police/panic-detail.php?id=' + alertId;
}
